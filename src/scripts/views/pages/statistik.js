...
const StatistikPage = {
  async render() {
    return `
      <h2 class="text-center mt-5" id="statistik">
        Statistik Konservasi Penyu
      </h2>
      <div class="row row-cols-1 row-cols-md-5 g-4 mx-5 my-2">
        <div class="col">
          <div
            class="card statistic-card text-center rounded-5"
      ...
      <!-- JENIS -->
      <h3 class="text-center" id="statistikJenis">
        Statistik Berdasarkan Jenis
      </h3>
      <div class="row row-cols-1 row-cols-md-2 g-4 mx-5 my-2">
        <div class="col">
          <div
            class="card text-center mx-auto  rounded-4 border-0"
            style="max-width: 30rem;"
          >
            <img
              src="https://i.ibb.co/K5fTpwX/lekang3.jpg"
              alt="Penyu-Lekang"
              border="0"
              class="rounded-4"
            />
            <div class="card-body">
              <button
                class="btn filled-button me-4 mt-3 px-3 rounded-4"
                data-bs-toggle="modal"
                data-bs-target="#detailPenyuLekang"
                type="button"
              >
                Penyu Lekang
              </button>
        ...
        <div class="col">
          <div
            class="card text-center mx-auto tipe_penyu rounded-4 border-0"
            style="max-width: 30rem;"
          >
            <img
              src="https://i.ibb.co/6b3dGzZ/Penyu-Sisik.jpg"
              alt="Penyu-Sisik"
              border="0"
              class="rounded-4"
            />
            <div class="card-body">
              <button
                class="btn filled-button me-4 mt-3 px-3 rounded-4"
                data-bs-toggle="modal"
                data-bs-target="#detailPenyuSisik"
                type="button"
              >
                Penyu Sisik
              </button>
  ...
  async afterRender() {
    ...
    // Lekang
    const sarangDitemukanLekang = document.getElementById(
      'sarangDitemukanLekang',
    );
    const telurDitemukanLekang = document.getElementById(
      'telurDitemukanLekang',
    );
    ...
    const tukikDilepaskanLekang = document.getElementById(
      'tukikDilepaskanLekang',
    );

    // Sisik
    const sarangDitemukanSisik = document.getElementById(
      'sarangDitemukanSisik',
    );
    ...
    const tukikDilepaskanSisik = document.getElementById(
      'tukikDilepaskanSisik',
    );
    ...
    document.addEventListener(RENDER_EVENT, async () => {
      ...
      // Lekang sarang telur
      const queryLekang = query(
        coll,
        where('inputJenisPenyu01', '==', 'Lekang'),
      );
      ...
      // rilis lekang
      const queryLekangRilis = query(
        collection(database, 'rilis'),
        where('jenisPenyu', '==', 'Lekang'),
      );
      ...
      // rilis sisik
      const querySisikRilis = query(
        collection(database, 'rilis'),
        where('jenisPenyu', '==', 'Sisik'),
      );
